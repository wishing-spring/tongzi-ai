# -*- coding: utf-8 -*-
"""Φ-field Plexus — GuaYuan connection matrix · Hebbian · BFS query · zero-float"""
import os, sys
...
class PhiField:
    ...
    def reinforce(self, active_pairs: list[tuple]):
        """Co-activated gua pairs → Hebbian reinforcement"""
        self.tick += 1
    ...
    def decay(self):
        """Consecutive inactivity → weight-1"""
        remove_keys = []
    ...
    def query(self, src: int, max_hops: int = 3) -> list[tuple[int, int]]:
        """BFS: from src, sorted by weight, return (gua, cumulative weight)"""
        visited = {src}
        result = []
        frontier = [(src, 0)]
    ...
    def query_path(self, src: int, max_hops: int = 3) -> int:
        """Query path → XOR cumulative gua (context signal for L3)"""
...
if __name__ == '__main__':
    ...
    for _ in range(5):
        ...
        phi.decay()
        print(f"  tick{phi.tick}: {phi.size()} connections total_w={phi.total_weight()}")
    ctx = phi.query_path(0x1234567, max_hops=2)
    print(f"\nQuery 0x1234567 → ctx=0x{ctx:07X}")
    print(f"Strongest: {phi.hardest_connections(3)}")
