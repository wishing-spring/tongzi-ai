# -*- coding: utf-8 -*-
"""F₂ Native Word World — 657 words → 28-bit hash · XOR chain-jump · zero-template"""
import os, sys
sys.path.insert(0, os.path.dirname(__file__))
from guayuan import MASK28, hamming, gua_hash

# 657-character lexicon + multi-character phrases
WORDS_RAW = """
...
print(f"[WordWorld] {len(WORD_SET)} words loaded")


class WordWorld:
    """F₂ word world: 928 words → 28-bit hash, XOR chain-jump"""
    ...
    def xor_chain_jump(self, start: int, steps: int = 5) -> list[str]:
        """XOR chain-jump: anti-collapse via word index scrambling"""
        path = []
        current = start
    ...
    def speak(self, attractor: int, active_guas: list[int] = None) -> str:
        """F₂ native output: attractor + active guas → word chain"""
        ctx = attractor
        if active_guas:
